  ...
  void _onLoading(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return Dialog(
          child: Container(padding: EdgeInsets.all(10), child: new Row(
    ...
    return Scaffold(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              padding: EdgeInsets.fromLTRB(10*fem, AppCommonHelper.isTablet(context)?10:20*fem, 10*fem, 10*fem),
              width: double.infinity,
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Container(
                    margin: EdgeInsets.fromLTRB(10*fem,AppCommonHelper.isTablet(context)?1: 20*fem,10*fem, AppCommonHelper.isTablet(context)?1:10*fem),
                    width: double.infinity,
                    height: 60*fem,
                    decoration: BoxDecoration (
                  ...
                  Container(
                    margin: EdgeInsets.fromLTRB(10*fem, 0*fem, 10*fem, AppCommonHelper.isTablet(context)?1:10*fem),
                    width: double.infinity,
                    height: 60*fem,
                    decoration: BoxDecoration (
                      borderRadius: BorderRadius.circular(5*fem),
                  ...
                  if(x.length>0)
                    Container(
                      height: 100,
                      width: double.infinity,
                      margin: EdgeInsets.all(AppCommonHelper.isTablet(context)?1:5),
                      ...
                      child:  ListView.builder(
                              ...
                              itemBuilder: (context, index)  {
                                return Image.file(x.elementAt(index),height: 100,width: 100,);
                  ...
                  Center(
                    child: ElevatedButton(
                        ...
                        if (result == null) {
                          ...
                          result?.files.forEach((element) {
                            ...
                            if(element.extension=="mp4"){
                              _generateThumbnail(element.path.toString()).then((value) {
                               x.add(value);
                               setState(() {
                      ...
                      child:  Text("Select File",style: TextStyle(fontSize: AppCommonHelper.isTablet(context)?30:16),),
                    ),
                  ),
                ],
              ),
            ),
           Text("Select Category",style: TextStyle(fontSize: AppCommonHelper.isTablet(context)?30:14),),
           load? Container(height: AppCommonHelper.isTablet(context)?120: 85,
             margin: EdgeInsets.all(AppCommonHelper.isTablet(context)?1:10),
             decoration: BoxDecoration (
              ...
              children: [
                Expanded(
                    child: CategorySelectItem(
                      ...
                      isSelected: profileModel.user!.primaryInetrest!.id.toString() == selected,
                      title: profileModel.user!.primaryInetrest!.name.toString(),
                      icon: profileModel.user!.primaryInetrest!.image.toString(),
                    )),
                Expanded(
                    child: CategorySelectItem(
                      isSelected: profileModel.user!.secondaryInetrest!.id.toString() == selected,
                      onclick: () {
                        selected = profileModel.user!.secondaryInetrest!.id.toString();
                        setState(() {});
                      },
                      title: profileModel.user!.secondaryInetrest!.name.toString(),
                      icon: profileModel.user!.secondaryInetrest!.image.toString(),
                    )),
                profileModel.user!.secondaryInetrest!=null?  Expanded(
                    child: CategorySelectItem(
                      isSelected: profileModel.user!.thirdInetrest!.id.toString() == selected,
                      onclick: () {
                        selected = profileModel.user!.thirdInetrest!.id.toString();
                        setState(() {});
                      },
                      title: profileModel.user!.thirdInetrest!.name.toString(),
                      icon: profileModel.user!.thirdInetrest!.image.toString(),
                    )):Container(),
              ],
            ),
            ):Container(),
          Container(
            margin: EdgeInsets.fromLTRB(5*fem, AppCommonHelper.isTablet(context)?5:15*fem, 5*fem, 0*fem),
            child:  Row(
                ...
                children: [
                 ...
                 }, child:Text('I agree with Terms & Conditions and privacy policy',style: TextStyle(color: Colors.grey,fontSize: 10),)),
                ]
            )),
            Container(
              margin: EdgeInsets.fromLTRB(AppCommonHelper.isTablet(context)?50:20*fem, 15*fem, AppCommonHelper.isTablet(context)?50:16*fem, 0*fem),
              child: TextButton(
                onPressed: () {
                  if(titleController.text.isEmpty){
                   ...
                   showCustomSnackBar("Description Required",context);
                    ...
                    showCustomSnackBar("Please accept our Terms & Conditions",context);
                  }else{
                    _onLoading(context);
                    ...
                    postPresenter.addPost(registerModel).then((value) {
                      ...
                      showCustomSnackBar("Post Added Successfully",context,isError: false);
                ...
                child: Container(
                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: SafeGoogleFont (
                        'Lato',
                        fontSize: AppCommonHelper.isTablet(context)?18: 20*ffem,
  ...
  Widget _buildAboutDialog(BuildContext context) {
    return new AlertDialog(
        ...
        content: new Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Text(AppConstants.terms,style: TextStyle(fontSize: 8, color: Colors.grey),)
          ],
        ),
        actions: <Widget>[
    new ElevatedButton(
